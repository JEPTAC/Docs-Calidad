...
try{
  ...
  await page.evaluate(()=>{
    ...
    if(typeof setMode==='function')setMode('word');
    else document.querySelector('[data-mode="word"]')?.click();
  });
  await page.waitForSelector('#wordSectionEditor',{timeout:10000});
  await page.waitForFunction(()=>window.V76_INTERACTIONS_READY===true,{timeout:10000});
  await page.waitForSelector('[data-v75-open-insert]',{timeout:10000});
  ...
  await page.waitForFunction(n=>(getWordItem(0,-1)?.blocks?.length||0)>n,before);
  ...
  await page.waitForSelector('[data-v75-open-diagram]',{timeout:10000});
  await page.locator('[data-v75-open-diagram]').last().click({force:true});
  await page.waitForFunction(()=>document.getElementById('wordContextDrawer')?.classList.contains('open'),{timeout:10000});
  await page.waitForSelector('#v75ContextBody .word-diagram-builder',{timeout:10000});
  ...
  console.log('UI smoke PASS: insertar, constructor visual, panel y enfoque.');
} finally {
  await browser.close();
}
