...
let doc={
  ...
  sections:[
    {n:'1',t:'OBJETIVO',c:'Definir el objetivo del documento.',sub:[]},
    {n:'2',t:'ALCANCE',c:'Establecer el alcance de aplicación.',sub:[]},
    {n:'3',t:'CONTENIDO',c:'Desarrollar el contenido técnico del documento.',sub:[
...
function setDefaultSectionsForWordType(t){
  ...
  doc.sections=presets.map(([n,t])=>({n,t,c:'',sub:[]}));
  render();
}
function addTypedWordSection(title){
  doc.sections=doc.sections||[];
  doc.sections.push({n:String(doc.sections.length+1),t:title,c:'',sub:[]});
  render();
}
...
function sgcPages(){
  ensureWordSubtitles();
  const toc=`<div class="page">${sgcHeader(1)}<div class="sgc-content"><h3 class="center" style="color:#001F73">TABLA DE CONTENIDO</h3>${sgcTocHtml()}</div>${sgcFooter(1)}</div>`;
  const content=`<div class="page">${sgcHeader(2)}<div class="sgc-content">${sgcSectionsHtml()}</div>${sgcFooter(2)}</div>`;
  return toc+content;
}
...
function cleanSectionNumber(n,fallback=''){
...
function nextSubNumber(i){
  ensureWordSubtitles();
  const sec=doc.sections[i];
  const base=cleanSectionNumber(sec?.n,i+1);
  return `${base}.${(sec?.sub||[]).length+1}`;
}
function renumberSubsections(i){
  ensureWordSubtitles();
  const sec=doc.sections[i];
  if(!sec)return;
  const base=cleanSectionNumber(sec.n,i+1);
  sec.sub=(sec.sub||[]).map((ss,j)=>({...ss,n:`${base}.${j+1}`}));
}
function addWordSubsection(i){
  const n=nextSubNumber(i);
  const sec=doc.sections[i];
  if(!sec)return;
  sec.sub=sec.sub||[];
  sec.sub.push({n,t:'Nuevo subtítulo',c:''});
  render();
}
function removeWordSubsection(i,j){
  ensureWordSubtitles();
  const sec=doc.sections[i];
  if(!sec||!sec.sub)return;
  sec.sub.splice(j,1);
  renumberSubsections(i);
  render();
}
function wordContentPage(){
  return 2;
}
function sgcTocHtml(){
  ensureWordSubtitles();
  return doc.sections.map((s,i)=>{
    ...
    const secRow=`<p class="sgc-toc-row"><span><b>${esc(secNo)}</b>&nbsp;&nbsp;${esc(s.t)}</span><span>${wordContentPage()}</span></p>`;
    const subRows=(s.sub||[]).map((ss,j)=>`<p class="sgc-toc-row sgc-toc-sub"><span><b>${esc(cleanSectionNumber(ss.n,`${secNo}.${j+1}`))}</b>&nbsp;&nbsp;${esc(ss.t)}</span><span>${wordContentPage()}</span></p>`).join('');
    return secRow+subRows
  }).join('');
}
function sgcSectionsHtml(){
  ensureWordSubtitles();
  return doc.sections.map((s,i)=>{
    const secNo=cleanSectionNumber(s.n,i+1);
