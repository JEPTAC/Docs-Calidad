...
function v76OwnEvent(e,fn){e.preventDefault();e.stopImmediatePropagation();try{fn()}catch(err){window.V76_LAST_ERROR=String(err?.stack||err);console.error('[Word Studio V77]',err)}}
...
function v76EnsureReady(){if(typeof v75EnsureShell==='function')v75EnsureShell();if(typeof v75SyncMode==='function')v75SyncMode();if(typeof v75UpdateToolbarState==='function')v75UpdateToolbarState();v76EnsureInsertPalette();const insert=document.getElementById('v75InsertBtn');if(insert)insert.title='Insertar contenido eligiendo sección o subtítulo';const panel=document.getElementById('v75PanelBtn');if(panel)panel.title='Mostrar u ocultar el panel lateral';const focus=document.getElementById('v75FocusBtn');if(focus)focus.title='Modo enfoque para trabajar sobre el documento'}
function v76HandleClick(e){
  ...
  const insertType=t.closest('[data-v76-insert-type]');if(insertType)return v76OwnEvent(e,()=>{if(!v76InsertTarget)return;const item=typeof getWordItem==='function'?getWordItem(v76InsertTarget.i,v76InsertTarget.j):null;if(!item)return;if(typeof addWordBlock==='function')addWordBlock(v76InsertTarget.i,v76InsertTarget.j,insertType.dataset.v76InsertType);v76CloseInsert();window.V76_LAST_ACTION=`insert-${insertType.dataset.v76InsertType}`});
